...
grid on   
ti = sprintf('$l_2$ error of Distributed Filtered Hyperinterpolation, $n = %d$, $h\\in C^5$, $m = %d$, %s',Lv(end),m,QH);
title(ti,'interpreter','latex')
...
hold off
% save figure
sv_err = [sv_fig_dir 'errvssigma_dfhyp' '_sigma' num2str(numel(sigmav)) '_L' num2str(Lv(end)) '_N' num2str(Nev) '.eps'];
